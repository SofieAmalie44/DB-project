from django.http import JsonResponse
from django.views import View
from bson import ObjectId
from pymongo import MongoClient
import json

from django.views.decorators.csrf import csrf_exempt
from django.utils.decorators import method_decorator

# Connect to MongoDB
client = MongoClient("mongodb://localhost:27017/")
db = client["rpg_mongo"]
inventory_collection = db["inventory"]


def fix_id(doc):
    """Convert _id ObjectId to string for JSON output."""
    doc["_id"] = str(doc["_id"])
    return doc


@method_decorator(csrf_exempt, name="dispatch")
class MongoInventoryList(View):
    """GET all inventory entries / POST create new inventory entry"""

    def get(self, request):
        items = [fix_id(doc) for doc in inventory_collection.find()]
        return JsonResponse(items, safe=False)

    def post(self, request):
        body = json.loads(request.body)

        result = inventory_collection.insert_one(body)
        body["_id"] = str(result.inserted_id)

        return JsonResponse(body, status=201)


@method_decorator(csrf_exempt, name="dispatch")
class MongoInventoryDetail(View):
    """GET one inventory entry / PUT update / DELETE"""

    def get(self, request, inv_id):
        doc = inventory_collection.find_one({"id": int(inv_id)})

        if not doc:
            return JsonResponse({"error": "Inventory entry not found"}, status=404)

        return JsonResponse(fix_id(doc))

    def put(self, request, inv_id):
        body = json.loads(request.body)

        result = inventory_collection.update_one(
            {"id": int(inv_id)},
            {"$set": body}
        )

        if result.matched_count == 0:
            return JsonResponse({"error": "Inventory entry not found"}, status=404)

        updated = inventory_collection.find_one({"id": int(inv_id)})
        return JsonResponse(fix_id(updated))

    def delete(self, request, inv_id):
        result = inventory_collection.delete_one({"id": int(inv_id)})

        if result.deleted_count == 0:
            return JsonResponse({"error": "Inventory entry not found"}, status=404)

        return JsonResponse({"status": "Inventory entry deleted"})
